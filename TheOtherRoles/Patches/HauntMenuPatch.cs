...
public static class HauntMenuPatch
{
    ...
    public static void Postfix(HauntMenuMinigame __instance)
    {
        ...
        var roleInfo = target.GetRoleInfo();

        if (__instance.HauntTarget.IsDead())
        {
            __instance.FilterText.text = roleInfo.Name + " Ghost";
            return;
        }

        __instance.FilterText.text = roleInfo.Name;
    }
    ...
    public static void MatchesFilterPostfix(HauntMenuMinigame __instance, PlayerControl pc, ref bool __result)
    {
        ...
        {
            __result = (pc.Data.Role.IsImpostor || pc.IsKiller()) && pc.IsAlive();
        }
    }
    ...
    {
        if (GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.Normal) return;
        if (PlayerControl.LocalPlayer.IsImpostor() && !PlayerControl.LocalPlayer.Is(RoleId.Vampire))
