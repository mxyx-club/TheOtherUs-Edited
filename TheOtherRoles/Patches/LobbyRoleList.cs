...
public static class LobbyRoleInfo
{
    ...
    public static void roleInfosOnclick(RoleType teamId)
    {
        ...
        List<Transform> buttons = new();
        int count = 0;
        foreach (var info in RoleInfo.AllRoleInfo.Where(x => x.RoleType != RoleType.Special))
        {
            if (info.RoleType == RoleType.Modifier && teamId != RoleType.Modifier) continue;
            else if (info.RoleType == RoleType.Neutral && teamId != RoleType.Neutral) continue;
            else if (info.RoleType == RoleType.Impostor && teamId != RoleType.Impostor) continue;
            else if (info.RoleType == RoleType.Crewmate && teamId != RoleType.Crewmate) continue;
            else if (info.RoleType == RoleType.Ghost && teamId != RoleType.Ghost) continue;

            Transform buttonTransform = UObject.Instantiate(buttonTemplate, container.transform);
            buttonTransform.name = Cs(info.Color, info.Name) + " Button";
            buttonTransform.GetComponent<BoxCollider2D>().size = new Vector2(2.5f, 0.55f);
            TextMeshPro label = UObject.Instantiate(textTemplate, buttonTransform);
            ...
            label.text = Cs(info.Color, info.Name);
            label.alignment = TextAlignmentOptions.Center;
            label.transform.localPosition = new Vector3(0, 0, label.transform.localPosition.z);
    ...
    public static void AddInfoCard(RoleInfo info)
    {
        string roleSettingDescription = info.FullDescription != "" ? info.FullDescription : info.ShortDescription;
        ...
        infoTitleText.text = Cs(info.Color, info.Name);
