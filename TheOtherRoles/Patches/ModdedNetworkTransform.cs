...
public static class ModdedNetworkTransform
{
    ...
    // Constants
    // 移動データの一括送信間隔
    private static float SEND_TIMER_THRESHOLD => SEND_TIMER_THRESHOLD_LEVELS[(byte)CustomOptionHolder.NetworkTransformType.GetSelection()];
    // 移動データ保存をスキップする最初の時間
    ...
    {
        var currentVelocity = player.MyPhysics.body.velocity;
        var isCurrentlyStopped = currentVelocity.sqrMagnitude < VELOCITY_ZERO_THRESHOLD_SQR; // Use squared magnitude for efficiency
        var wasMovingLastFrame = lastVelocity.sqrMagnitude >= VELOCITY_ZERO_THRESHOLD_SQR;
        var isCountingDown = stopDetectionCounter.ContainsKey(player.PlayerId); // No need for TryGetValue here
    ...
    {
        var currentStopCount = stopDetectionCounter.TryGetValue(player.PlayerId, out var count) ? count : 0;

        if (wasMovingLastFrame || isCountingDown) // Start stopping or continue countdown
        {
            var nextCount = currentStopCount + 1;
    ...
    private static void HandleOwnerMove(PlayerControl player, Vector2 currentVelocity, bool isCountingDown)
    {
        ...
        var startedMoving = !isCountingDown && lastVelocity.sqrMagnitude < VELOCITY_ZERO_THRESHOLD_SQR;

        // Significant direction change check
        var directionChangedSignificantly = !startedMoving &&
        ...
        else // Continuing movement or changed direction
        {
            if (directionChangedSignificantly)
            {
                ...
                Info($"Direction Change: sendTimer = {sendTimer}, SEND_TIMER_THRESHOLD = {SEND_TIMER_THRESHOLD}, {(SEND_TIMER_THRESHOLD - sendTimer) / 3f}");
    ...
    public static void ReceivedNetworkTransform(MessageReader reader)
    {
        ...
        switch (rpcType)
        {
            case (byte)MovementRpcType.BatchMovement:
                ...
                var count = Mathf.Min(reader.ReadInt32(), 20); // Read count, capped at 20
            ...
            case (byte)MovementRpcType.ApplyExternalImpulse:
                playerId = reader.ReadByte();
                var impulse = new Vector2(reader.ReadSingle(), reader.ReadSingle());
    ...
    private static void FixedUpdateRemote(PlayerControl player)
    {
        ...
        if (externalImpulses.Remove(player.PlayerId, out var impulse) && impulse.sqrMagnitude > 0.0001f) // C# 7.0+
        {
            ...
            // インパルスの方向に少しだけ位置を補正
            var adjustmentMagnitude = Mathf.Min(impulse.magnitude * Time.fixedDeltaTime * 2.0f, 0.1f);
            var positionAdjustment = (Vector3)impulse.normalized * adjustmentMagnitude;
    ...
    private static bool HandleRemoteSmoothingStop(PlayerControl player)
    {
        ...
        if (!smoothingTimeLeft.TryGetValue(player.PlayerId, out var remainingTime))
        {
        ...
        if (!smoothingVelocity.TryGetValue(player.PlayerId, out var vel)) // Calculate velocity only once or if not present
        {
            ...
            var timeForCalc = remainingTime > 0.001f ? remainingTime : STOP_SMOOTHING_TIME; // Fallback to STOP_SMOOTHING_TIME
    ...
    private static bool HandleRemoteMovementPlayback(PlayerControl player)
    {
        if (movementQueues.TryGetValue(player.PlayerId, out var queue) && queue.Count > 0)
        {
            ...
            var targetPosition3D = new Vector3(record.position.x, record.position.y, player.transform.position.z);
            var distance = Vector2.Distance(player.transform.position, record.position);

            // Interpolate position smoothly
            var speed = record.velocity.magnitude;
            // Avoid division by zero or very small update steps for lerp factor
            var lerpFactorBase = speed * Time.fixedDeltaTime > 0.01f ? Mathf.Clamp01(distance / (speed * Time.fixedDeltaTime)) : 1.0f;
            ...
            // Position Sync Logic
            var currentSyncLimit = syncPositionLimit.TryGetValue(player.PlayerId, out var limit) ? limit : SYNC_POSITION_LIMIT_INITIAL_VALUE;
            currentSyncLimit--;
    ...
    public static class CustomNetworkTransformOnEnablePatch
    {
        public static void Postfix(CustomNetworkTransform __instance)
        {
            ...
            if (CustomOptionHolder.NetworkTransformLowLevel.GetBool())
    ...
    {
        public static bool IsVanillaServer() => AmongUsClient.Instance.NetworkMode != NetworkModes.LocalGame && !IsCustomServer();
        public static bool Prefix(CustomNetworkTransform __instance)
        {
            // 開始時のみ
            if (AmongUsClient.Instance.GameState != InnerNetClient.GameStates.Started) return true;
