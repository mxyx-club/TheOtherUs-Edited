...
public static class VentCanUsePatch
{
        ...
        [HarmonyArgument(1)] ref bool canUse, [HarmonyArgument(2)] ref bool couldUse)
    {
        ...
        {
            if (!PlayerControl.LocalPlayer.Is(RoleId.Trickster))
            {
...
internal class VentButtonDoClickPatch
{
    private static bool Prefix(VentButton __instance)
    {
        ...
        if (__instance.currentTarget != null && !CustomRoleManager.handcuffedKnows.ContainsKey(PlayerControl.LocalPlayer.PlayerId))
...
public static class JesterEnterVent
{
    ...
    {
        return !PlayerControl.LocalPlayer.Is(RoleId.Jester);
    }
}
...
public static class VentUsePatch
{
    public static bool Prefix(Vent __instance)
    {
        ...
        if (CustomRoleManager.handcuffedPlayers.Contains(PlayerControl.LocalPlayer.PlayerId))
        {
            CustomRoleManager.setHandcuffedKnows();
            return false;
        }

        __instance.CanUse(PlayerControl.LocalPlayer.Data, out var canUse, out var couldUse);
        var canMoveInVents = !PlayerControl.LocalPlayer.Is(RoleId.Spy);
...
internal class VentButtonVisibilityPatch
{
    private static void Postfix(HudManager __instance)
    {
        if (PlayerControl.LocalPlayer?.AmOwner == true && ShowButtons)
        {
            ...
            if (ShowButtons)
            {
                if (PlayerControl.LocalPlayer.CanUseVents())
                    __instance.ImpostorVentButton.Show();

                if (PlayerControl.LocalPlayer.CanUseSabotage())
                {
...
internal class VentButtonSetTargetPatch
{
    ...
    private static void Postfix(VentButton __instance)
    {
        ...
        if (PlayerControl.LocalPlayer.Is(RoleId.Tunneler))
...
internal class ReportButtonDoClickPatch
{
    ...
    {
        if (__instance.isActiveAndEnabled && CustomRoleManager.handcuffedPlayers.Contains(PlayerControl.LocalPlayer.PlayerId) &&
            __instance.graphic.color == Palette.EnabledColor) CustomRoleManager.setHandcuffedKnows();
        return !CustomRoleManager.handcuffedKnows.ContainsKey(PlayerControl.LocalPlayer.PlayerId);
    }
}
...
internal class EmergencyMinigameUpdatePatch
{
    private static bool CanCallEmergency(out string statusText)
    {
        ...
        if (PlayerControl.LocalPlayer.Is(RoleId.Swapper) && !Swapper.canCallEmergency)
        {
            statusText = GetString("swapperMeetingButton");
            return false;
        }

        if (PlayerControl.LocalPlayer.Is(RoleId.Jester) && !Jester.canCallEmergency)
        {
            statusText = GetString("jesterMeetingButton");
            return false;
        }

        if (PlayerControl.LocalPlayer.Is(RoleId.Prosecutor) && !Prosecutor.canCallEmergency)
        {
            statusText = GetString("prosecutorMeetingButton");
            return false;
        }

        if (PlayerControl.LocalPlayer.Is(RoleId.Lawyer) && !Lawyer.canCallEmergency)
        {
            statusText = GetString("lawyerMeetingButton");
            return false;
        }

        if (PlayerControl.LocalPlayer.Is(RoleId.Executioner) && !Executioner.canCallEmergency)
        {
            statusText = GetString("executionerMeetingButton");
            return false;
        }

        if (PlayerControl.LocalPlayer.Is(RoleId.Prophet) && !Prophet.canCallEmergency)
        {
    ...
    private static void Postfix(EmergencyMinigame __instance)
    {
        ...
        if (__instance.state == 1)
        {
            ...
            var remaining = Mathf.Min(localRemaining, PlayerControl.LocalPlayer.Is(RoleId.Mayor) ? 1 : teamRemaining);
...
public static class ConsoleCanUsePatch
{
    ...
    {
        canUse = couldUse = false;
        if (PlayerControl.LocalPlayer.Is(RoleId.Swapper) && !Swapper.canFixSabotages)
...
internal class CommsMinigameBeginPatch
{
    private static void Postfix(TuneRadioMinigame __instance)
    {
        ...
        if (PlayerControl.LocalPlayer.Is(RoleId.Swapper) && !Swapper.canFixSabotages) __instance.Close();
    }
}
...
internal class VitalsMinigamePatch
{
    ...
    private class VitalsMinigameStartPatch
    {
        ...
        {
            if (PlayerControl.LocalPlayer.Is(RoleId.Hacker))
            {
    ...
    private class VitalsMinigameUpdatePatch
    {
        private static void Postfix(VitalsMinigame __instance)
        {
            ...
            if (PlayerControl.LocalPlayer.TryGetRole<Hacker>(out var hacker) && hacker.hackerTimer > 0)
                for (var k = 0; k < __instance.vitals.Length; k++)
                {
                    ...
                    {
                        var deadPlayer = PlayerData.AllPlayerData.Values?.Where(x => x.Player?.PlayerId == player?.PlayerId)?.FirstOrDefault();
...
internal class AdminPanelPatch
{
    ...
    private class CounterAreaUpdateCountPatch
    {
        ...
        private static void Postfix(CounterArea __instance)
        {
            ...
            var showHackerInfo = PlayerControl.LocalPlayer.TryGetRole<Hacker>(out var hacker) && hacker.hackerTimer > 0;
...
internal class SurveillanceMinigamePatch
{
        ...
        PlanetSurveillanceMinigame SwitchCamsMinigame = null, FungleSurveillanceMinigame FungleCamMinigame = null)
    {
        ...
        if (isLightsOut && !nightVisionIsActive && nightVisionEnabled && !ignoreNightVision)
        {
            ...
            {
                if (pc.GetRole<Ninja>()?.invisibleTimer > 0f) continue;
                pc.setLook("", 11, "", "", "", "", false);
            }
    ...
    public static void resetNightVision()
    {
        ...
        if (nightVisionIsActive)
        {
            ...
            foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
            {
                if (pc.TryGetRole<Morphling>(out var morphling) && morphling.morphTimer > 0)
                {
                ...
                else if (pc.GetRole<Ninja>()?.invisibleTimer > 0f)
                {
                    continue;
                }
                else
