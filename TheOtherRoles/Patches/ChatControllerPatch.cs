...
public static class ChatControllerPatch
{
    ...
    public static ChatTypes CurrentChatType = ChatTypes.None;
    ...
    public static class SetBubbleName
    {
        private static void Postfix(ChatBubble __instance, [HarmonyArgument(0)] string playerName)
        {
            ...
            var localJailor = PlayerControl.LocalPlayer.GetRole<Jailor>();
            var sourceJailor = sourcePlayer?.GetRole<Jailor>();

            if (InMeeting && localJailor != null && localJailor.Jailed != null && localJailor.Jailed.IsAlive())
            {
                if (localJailor.Jailed == PlayerByName(playerName))
                {
                    __instance.NameText.color = Jailor.color;
                    __instance.NameText.text = playerName + GetString("Jailor.InJailSuffix");
                }

                if ((PlayerControl.LocalPlayer == localJailor.Jailed || CanSeeGhostInfo) &&
                    localJailor.Jailed == PlayerByName(playerName))
                {
                    __instance.NameText.color = Jailor.color;
                    __instance.NameText.text = playerName + GetString("Jailor.InJailSuffix");
                }
            ...
            switch (CurrentChatType)
            {
                case ChatTypes.HostChat:
                    ...
                    CurrentChatType = ChatTypes.None;
                    break;
                case ChatTypes.GhostChat:
                    break;
                case ChatTypes.JailorChat:
                    if (InMeeting && localJailor != null && localJailor.Jailed != null && localJailor.Jailed.IsAlive())
                    {
                        ...
                        else if (PlayerControl.LocalPlayer == localJailor.Player)
                        {
                            __instance.NameText.color = Jailor.color;
                            __instance.NameText.text = $"({GetString("Jailor")})";
                            CurrentChatType = ChatTypes.None;
    ...
    private static class AddChatPatch
    {
        private static bool Prefix(ChatController __instance, [HarmonyArgument(0)] PlayerControl sourcePlayer, ref bool __state)
        {
            ...
            if (__instance != FastDestroyableSingleton<HudManager>.Instance.Chat)
                return true;
            if (Blackmailer.IsBlackmailed(sourcePlayer))
            { __state = false; return false; }
            if (Jailor.IsJailed(sourcePlayer, out var jailor) && jailor != PlayerControl.LocalPlayer && !CanSeeGhostInfo)
            { __state = false; return false; }
    ...
    public static void Initialize()
    {
        ...
        ChatCommandRegistry.Register(["setrole", "sr"], (sender, args, chat) =>
        {
            ...
            else if (args.Length == 1 && args[0].Equals("ls", StringComparison.OrdinalIgnoreCase))
            {
                ...
                foreach (var info in RoleInfo.AllRoleInfo.Where(x => x.RoleType == RoleType.Crewmate))
                {
                ...
                foreach (var info in RoleInfo.AllRoleInfo.Where(x => x.RoleType == RoleType.Neutral))
                {
                ...
                foreach (var info in RoleInfo.AllRoleInfo.Where(x => x.RoleType == RoleType.Impostor))
                {
        ...
        ChatCommandRegistry.Register("m", (sender, args, chat) =>
        {
            ...
            var localRole = CustomRoleManager.AllRoles.Where(x => x.Player == PlayerControl.LocalPlayer).Select(x => x.RoleInfo);
        ...
        {
            if (sender.TryGetRole<Jailor>(out var jailor) && sender.IsAlive() && jailor.Jailed != null && InMeeting)
            {
        ...
        static RoleId GetRoleId(string[] args = null)
        {
            ...
            var infos = RoleInfo.AllRoleInfo.Where(x => x.Name.StartsWith(args[0], StringComparison.OrdinalIgnoreCase));
            if (infos.Count() == 1)
            ...
            else if (infos.Count() > 1)
            {
                ...
                {
                    sb.AppendLine($"• {(int)info.RoleId} - {info.Name}");
                }
                Message(sb.ToString());
                return RoleId.DefaultRole;
            }
