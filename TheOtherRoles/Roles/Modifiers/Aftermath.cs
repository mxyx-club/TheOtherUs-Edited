...
public class Aftermath : ModifierBase
{
    ...
    public Aftermath(PlayerControl p) : base(p, roleinfo) { }

    public override RoleType[] RemoveTeam { get; set; } = [RoleType.Impostor];
    ...
    {
        if (Info.Target == Player && PlayerControl.LocalPlayer == Info.Target)
        {
            _ = new LateTask(() =>
            {
                afterTrigger(Player.PlayerId, Info.Killer.PlayerId);
