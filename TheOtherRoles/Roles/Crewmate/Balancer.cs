...
public class Balancer : RoleBase
{
    ...
    public Balancer(PlayerControl p) : base(p, roleinfo) { CustomRoleManager.OnFixedUpdateOthers.Add(OnFixedUpdate); }
    ...
    private static void AddOptions()
    {
        ...
        balancerCount = CustomOption.Create(configId++, CustomOptionType.Crewmate, "balancerCount", 1, 1, 3, 1, roleinfo.RoleOption);
        balancerVoteTime = CustomOption.Create(configId++, CustomOptionType.Crewmate, "balancerVoteTime", 40, 15, 90, 5, roleinfo.RoleOption);
    }

    public override void Initialize()
    {
        IsAbilityUsed = balancerCount.GetInt();
        BalancerVoteTime = balancerVoteTime.GetInt();
    ...
    public static RemoteProcess<(PlayerControl player, PlayerControl target1, PlayerControl target2)> BalancerBalance = new("BalancerBalance", (data, _) =>
    {
        StartAbility(data.player, data.target1, data.target2);
    });
    ...
    {
        if (player == currentTarget) currentTarget = null;
    }
    ...
    {
        MeetingHud.Instance.discussionTimer = GameOptionsManager.Instance.CurrentGameOptions.GetInt(AmongUs.GameOptions.Int32OptionNames.VotingTime) - BalancerVoteTime - 6.5f;
    ...
    private void BalancerOnClick(PlayerVoteArea pva, MeetingHud __instance)
    {
        ...
        if (PlayerControl.LocalPlayer.IsDead() || target.IsDead() || IsAbilityUsed <= 0) return;

        BalancerBalance.Invoke((PlayerControl.LocalPlayer, currentTarget, target));
    ...
    {
        if (Player.IsAlive() && IsAbilityUsed > 0)
        {
            ...
            foreach (var pva in __instance.playerStates)
            {
                ...
                if (player.IsAlive())
                {
                    ...
                    button.OnClick.AddListener((UnityAction)(() => BalancerOnClick(pva, __instance)));
                }
            }
        }
    }
    ...
    {
        if (Player != null && (Player.IsDead() || currentAbilityUser != null))
        {
            __instance.playerStates.ForEach(x =>
            {
                var icon = x.transform.FindChild("BalancerButton");
                if (icon?.gameObject != null) UObject.Destroy(icon.gameObject);
            });
        }
    }
}
