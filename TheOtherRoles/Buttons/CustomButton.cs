...
public class CustomButton
{
    ...
    public float MaxTimer
    {
        get
        {
            ...
            if (IsKillButton)
            {
                ...
                if (local.IsImpostor() && local.Is(RoleId.LastImpostor))
    ...
    public void onClickEvent()
    {
        ...
        if (CustomRoleManager.handcuffedKnows.ContainsKey(PlayerControl.LocalPlayer.PlayerId)
            && CustomRoleManager.handcuffedKnows[PlayerControl.LocalPlayer.PlayerId] > 0f)
            return;
    ...
    public void Update()
    {
        ...
        if ((!InGame || Started) && Timer >= 0 && ((HasEffect && isEffectActive) || !local.inVent))
        ...
        if (CustomRoleManager.handcuffedPlayers.Contains(local.PlayerId))
            OnClick = () => CustomRoleManager.setHandcuffedKnows();
        else
            OnClick = InitialOnClick;
    }

    public void showTargetNameOnButton(PlayerControl target, string defaultText = "")
    {
        if (CustomOptionHolder.showButtonTarget.GetBool())
        {
            defaultText = defaultText.IsNullOrWhiteSpace() ? buttonText : defaultText;
            ...
            text = target == null ? defaultText : target?.Data?.PlayerName ?? "";
        ...
        {
            actionButton.OverrideText(defaultText);
            showButtonText = true;
        }
    }
    /// <summary>
    ...
    public static void setAllButtonsHandcuffedStatus(bool handcuffed, bool reset = false)
    {
        ...
        if (handcuffed && !deputyHandcuffedButtons.ContainsKey(PlayerControl.LocalPlayer.PlayerId))
        {
            ...
            foreach (var t in Buttons)
            {
                try
                {
                    // For each custombutton the player has
                    if (t.HasButton()) addReplacementHandcuffedButton(t);
        ...
        static void addReplacementHandcuffedButton(CustomButton button, Func<bool> couldUse = null)
        {
            ...
            var replacementHandcuffedButton = new CustomButton(() => { }, () => { return true; }, couldUse, () => { },
                Deputy.handcuffedSprite, button.hudManager, button.textTemplate, null,
                true, Deputy.handcuffDuration, null, null, null, button.mirror);
