...
public static class FungleAdditionalElectrical
{
    public static void CreateElectrical()
    {
        ...
        Console console1 = UObject.Instantiate(MapLoader.Airship.transform.FindChild("Storage/task_lightssabotage (cargo)"), fungleShipStatus.transform).GetComponent<Console>();
        console1.transform.localPosition = new(-16.2f, 7.67f, 0);
        console1.ConsoleId = 0;

        Console console2 = UObject.Instantiate(MapLoader.Skeld.transform.FindChild("Electrical/Ground/electric_frontset/SwitchConsole"), fungleShipStatus.transform).GetComponent<Console>();
    ...
    private class MapBehaviourAwakePatch
    {
        public static void Postfix(MapBehaviour __instance)
        {
            ...
            MapRoom mapRoom = UObject.Instantiate(MapLoader.Airship.MapPrefab.infectedOverlay.rooms.FirstOrDefault(x => x.room == SystemTypes.Electrical), __instance.infectedOverlay.transform);
