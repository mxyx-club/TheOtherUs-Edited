...
public class Trap : CustomObjectBase<Trap>
{
    ...
    {
        var traps = AllObjects.Where(x => x.Player == trapper && (active || !x.revealed));
    ...
    public static void triggerTrap(byte targetId, int trapId)
    {
        ...
        if (t.Player == null || t == null || t.trapped.Contains(target) || target == null) return;

        t.usedCount++;
        t.triggerable = false;
        if (targetId == PlayerControl.LocalPlayer.PlayerId || targetId == t.Player.PlayerId)
        {
        ...
        if (PlayerControl.LocalPlayer.PlayerId == t.Player.PlayerId) t.arrow.arrow.SetActive(true);
        ...
        if (t.revealed && (PlayerControl.LocalPlayer == t.Player || CanSeeGhostInfo))
        {
            ...
            message = t.trapped.Aggregate(message, (current, p) => current + Trapper.infoType switch
            {
                ...
                1 when (p.IsImpostor() || p.IsEvilNeutral() || p.IsKillerNeutral()) ^ Vortox.Reversal => "邪恶职业 \n",
                1 => "善良职业 \n",
                _ => p.Data.PlayerName + "\n"
            });

            FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(t.Player, $"{message}");
        }
    }
    ...
    public void Update(PlayerControl player)
    {
        ...
        var canSee = CanSeeGhostInfo || PlayerControl.LocalPlayer == Player || trapped.Any(x => x.PlayerId == player.PlayerId);
        ...
        if (distance < 0.6f)
        {
            if (!revealed && player.PlayerId != Player.PlayerId && player.IsAlive())
            {
