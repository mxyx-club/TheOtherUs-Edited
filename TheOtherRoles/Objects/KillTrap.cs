...
public class KillTrap : CustomObjectBase<KillTrap>
{
    ...
    public static RemoteProcess<(PlayerControl trapper, PlayerControl target, int trapId)> TrapKillRpc = new("trapKill", (data, _) =>
    {
        var trap = AllObjects.FirstOrDefault(x => x.Id == data.trapId);
        ...
        if (PlayerControl.LocalPlayer == data.trapper) RpcCustomMurderPlayer(data.trapper, data.target, false);

        //EvilTrapper.isTrapKill = true;
    });

    public static RemoteProcess<int> DisableTrapRpc = new("disableTrap", (data, _) =>
    {
        var trap = AllObjects.FirstOrDefault(x => x.Id == data);
        trap.isTriggered = false;
        trap.isDisabled = true;
        trap.audioSource.Stop();
        trap.audioSource.PlayOneShot(disable);
        new LateTask(trap.Destroy, disable.length + 1f, "Destroy KillTrap");
    });


    public static RemoteProcess<(PlayerControl player, PlayerControl target, int trapId)> ActivateTrapRpc = new("disableTrap", (data, _) =>
    {
        activateTrap(data.player, data.target, data.trapId);
    });
        ...
        HudManager.Instance.StartCoroutine(Effects.Lerp(EvilTrapper.killTimer, new Action<float>((p) =>
        {
            try
            {
                ...
                else if ((p == 1f) && !target.Data.IsDead)
                {
                    ...
                    {
                        TrapKillRpc.Invoke((PlayerControl.LocalPlayer, target, trapId));
                    }
                }
                else
                { // カウントダウン中の処理
    ...
    public override void Update()
    {
        ...
        if (!hasTrappedPlayer() && !InMeeting)
        {
            ...
            if (distance < EvilTrapper.trapRange)
            {
                ...
                {
                    ActivateTrapRpc.Invoke((trapper, PlayerControl.LocalPlayer, Id));
                }
            }
        }
    }
